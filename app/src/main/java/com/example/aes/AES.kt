...
class AES {
    ...
    private fun mixColumns(state: Array<IntArray>): Array<IntArray> {
        ...
        for (c in 0 until Nb) {
            temp0 =
                mult(0x02, state[0][c]) xor mult(0x03, state[1][c]) xor state[2][c] xor state[3][c]
            temp1 = state[0][c] xor mult(0x02, state[1][c]) xor mult(
                0x03,
                state[2][c],
            ) xor state[3][c]
            temp2 = state[0][c] xor state[1][c] xor mult(0x02, state[2][c]) xor mult(
                0x03,
                state[3][c],
            )
            temp3 = mult(0x03, state[0][c]) xor state[1][c] xor state[2][c] xor mult(
                0x02,
                state[3][c],
            )
            state[0][c] = temp0
